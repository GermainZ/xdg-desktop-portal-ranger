...
void print_config(enum LOGLEVEL loglevel, struct xdpw_config *config) {
    logprint(loglevel, "config: term_cmd:  %s", config->filechooser_conf.term_cmd);
    logprint(loglevel, "config: filechooser_script:  %s", config->filechooser_conf.filechooser_script);
}
...
void finish_config(struct xdpw_config *config) {
    ...
    free(config->filechooser_conf.term_cmd);
    free(config->filechooser_conf.filechooser_script);
}
...
static int handle_ini_filechooser(struct config_filechooser *filechooser_conf, const char *key, const char *value) {
    if (strcmp(key, "term_cmd") == 0) {
        parse_string(&filechooser_conf->term_cmd, value);
    } else if (strcmp(key, "filechooser_script") == 0) {
        parse_string(&filechooser_conf->filechooser_script, value);
    } else {
...
static void default_config(struct xdpw_config *config) {
    config->filechooser_conf.term_cmd = "/usr/bin/kitty";
    config->filechooser_conf.filechooser_script = "/usr/share/xdg-desktop-portal-ranger/ranger-wrapper.sh";
}
...
static char *config_path(const char *prefix, const char *filename) {
    ...
    char *config_folder = "xdg-desktop-portal-ranger";
